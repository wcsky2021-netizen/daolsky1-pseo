...
const THEME_OVERRIDES: Record<string, number> = {
  // 다올스카이차: 별도 계정/Worker라 자동배정 시 site.id=1 → 블루(아자스카이와 동일 톤)가 됨.
  // footprint·인상 분리를 위해 그린/틸로 강제. 형님 취향에 맞게 0(핑크)/1(블루)로 변경 가능.
  // OG 이미지 색은 content-pipeline/theme.py 의 OG_THEME_OVERRIDES 인덱스와 맞출 것.
  'daolsky1.co.kr': 2,
};
